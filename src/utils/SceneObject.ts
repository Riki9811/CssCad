...
export class SceneObject implements SceneObjectInterface {
	...
	public getSubTreeNames(): string[] {
		let names = [this.name];
		for (const child of this.children) {
			names = names.concat(child.getSubTreeNames());
		}
		return names;
	}

	public copyWithoutChildren(): SceneObject {
		return new SceneObject(this.name, this.position, this.rotation, this.color, []);
	}
}
...
export class SceneObject2D extends SceneObject {
	...
	public copyWithoutChildren(): SceneObject2D {
		return new SceneObject2D(this.name, this.position, this.rotation, this.color, this.width, this.height, []);
	}
}
...
export class ObjectTree {
	...
	*preOrderTraversal(node: SceneObject = this.root): IterableIterator<SceneObject> {
	...
	/**
	 * Calls a defined callback function on each object of the tree,
	 * and returns an array that contains the results.
	 * @param func A function called by the map method one time for each element in the array
	 * @returns Array that contains the results of func called on each object.
	 */
	public map<T>(func: (obj: SceneObject) => T): T[] {
		let newArr: T[] = [];
		for (let obj of this.preOrderTraversal()) {
			if (obj.name !== "root") newArr.push(func(obj));
		}
		return newArr;
	}
	//#endregion
	...
	public preventNameDuplicates(objName: string): string {
		let names = this.root.getSubTreeNames();
		let newName = objName;
		let iteration = 1;
	 ...
	 * @returns The name of the object inserted (undefined in case operation fails)
	 */
	public insert(obj: SceneObject, parentName?: string): string | undefined {
	...
	public moveInto(objToMove: SceneObject, parentName?: string) {
		if (parentName) {
			let names = objToMove.getSubTreeNames();
			if (names && names?.includes(parentName)) return;
			this.remove(objToMove.name);
			this.insert(objToMove, parentName);
		} else {
			this.remove(objToMove.name);
			this.insert(objToMove);
		}
	}
	//#endregion
